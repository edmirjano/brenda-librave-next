...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!slug) {
      return NextResponse.json({ success: false, error: 'Book slug is required' }, { status: 400 });
    }
    ...
    if (!book.active) {
      return NextResponse.json({ success: false, error: 'Book not available' }, { status: 404 });
    }
