...
test.describe('Health Check and Basic Navigation', () => {
  ...
  test('homepage loads correctly with Albanian content', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1 })).toContainText('Mirë se vini në Brënda Librave');
  ...
  test('page has proper meta tags for SEO', async ({ page }) => {
    ...
    await expect(metaDescription).toHaveAttribute('content', expect.stringContaining('libra shqiptarë'));
